...
int main(int argc, char **argv) {
	...
	while (1)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			if (bouncer_x < 100 || bouncer_x > SCREEN_W - BOUNCER_SIZE) {
				bouncer_dx = -bouncer_dx;
			}

			if (bouncer_y < 100 || bouncer_y > SCREEN_H - BOUNCER_SIZE) {
			...
			//here's the parametric equations that determine the shape!!
				...
				bouncer_x = 300 + 5 * (26 * cos(t) - 24 * cos(2.4*t));
			bouncer_y = 250 + 5 * (52 * sin(t) - 24 * sin(2.4*t));
